...
public class Question7Handler : QuestionHandler
{
    ...
    void Start()
    {
        ...
        bagRock = VisualTestUI.transform.GetChild(4).gameObject.transform.GetChild(7).gameObject;
    }
    ...
    void Update()
    {
        ...
        if (parta.GetComponent<ItemSlot>().isAccepted() && partb.GetComponent<ItemSlot>().isAccepted() && partc.GetComponent<ItemSlot>().isAccepted())
        {
            ...
            if (questionDone)
                VisualTestUI.GetComponent<UIHandler>().CancelQuestion7();
            else
